if [ -z "$CORE" ]; then
    ...
    cp /sql/New_Install_Core/world_content.sql /docker-entrypoint-initdb.d/2.sql
## Else we'll default to the demo
else
    echo "Copying NEW Demo SQL files to  /docker-entrypoint-initdb.d/2.sql"
    cp /sql/New_Install_with_Demo_Data/world_content.sql /docker-entrypoint-initdb.d/2.sql
## End if
fi
